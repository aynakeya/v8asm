...
- 11.4 Node-style cache 目录使用
  ...
  build args 为 `v8_enable_pointer_compression=false v8_enable_sandbox=false`。
- 10.8 Electron-style cache 目录使用
  `tests/decomp_rounds/bin_cache/v8asm.10.8.electron.x64.release/`；对应 V8
  out 目录是
...
- When rebuilding the patch matrix, reuse the persistent row output directories
  listed in `tests/decomp_rounds/build_v8asm_matrix.sh` and the matching
  `tests/decomp_rounds/bin_cache/*` directories. Do not invent a temporary
  output directory for a known row; that discards cache value and weakens later
  reproducibility checks.
- Do not run `gclient sync -D` for this workflow. The official sync command is
  `gclient sync --with_branch_heads --with_tags`; if depot_tools prints that
  stale directories could be removed, leave them alone unless the user explicitly
  asks for cleanup.
- `tests/decomp_rounds/bin_cache/*/snapshot_blob.bin` must remain the snapshot
  generated by that exact V8 build. Electron, Node, Chromium, app, and
  `v8_context_snapshot.bin` inputs are passed explicitly with `--snapshot_blob`
  or the matrix override variables, never copied beside cached `v8asm`.
- If a new operational rule is discovered while debugging, update `README.md`,
  this file, and a dated note under `note/` before or with the fixing commit.
